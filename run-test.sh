...
while true; do
  ...
  while true; do
    ...
    if [[ "${exit_code}" != 0 ]]; then
      ...
      mosquitto_pub -h "${MQTT_HOST}" -p "${MQTT_PORT}" -t "${MQTT_TOPIC_PREFIX}/last_seen" -m "${test_time}"
      mosquitto_pub -h "${MQTT_HOST}" -p "${MQTT_PORT}" -t "${MQTT_TOPIC_PREFIX}/ping_latency" -m "${latency}"
      mosquitto_pub -h "${MQTT_HOST}" -p "${MQTT_PORT}" -t "${MQTT_TOPIC_PREFIX}/download_speed" -m "${download}"
      mosquitto_pub -h "${MQTT_HOST}" -p "${MQTT_PORT}" -t "${MQTT_TOPIC_PREFIX}/upload_speed" -m "${upload}"
